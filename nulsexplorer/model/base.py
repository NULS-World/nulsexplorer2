...
class BaseClass(SerializerObject):
    ...
    def _collection_name(self):
        if getattr(self, "COLLECTION", None) is None:
            raise ValueError("Please specify collection name on classes inheriting from BaseClass")
    ...
    async def refresh(self):
        if not self._id:
            raise ValueError("You should save a new object before trying to update it.")
